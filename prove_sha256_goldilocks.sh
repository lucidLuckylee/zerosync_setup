...
TARGET="goldilocks_target"
mkdir ${TARGET}

# Compile the cairo program
cairo-compile main.cairo --output=${TARGET}/main_compiled.json --proof_mode --prime 18446744069414584321
# Start the runner to generate the air inputs
cairo-run --program=${TARGET}/main_compiled.json \
        --layout=all --print_output \
...
\time -f "%E %M" cargo +nightly run -r -F parallel,asm -- \
 ...
 --output ${TARGET}/sha256_goldilocks_main.proof
